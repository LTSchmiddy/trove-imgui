...
void drawTestTexture(ImGuiIO& io)
{
    ...
    ImGui::Image(imguiTextureId, ImVec2(my_tex_w, my_tex_h), uv_min, uv_max,
        tint_col, border_col);
}
...
int main(int, char**)
{
    ...
    // player.setVideoCallbacks(videoLockCallBack, videoUnlockCallback,
    // videoDisplayCallback);
    libvlc_video_set_callbacks(player.get(), videoLockCallBack,
        videoUnlockCallback, videoDisplayCallback, NULL);
    ;
    ...
    bool ret = LoadTextureFromFile("out.png", &test_image_texture,
        ...
        // to created a named window.
        {
            ...
            if (player.isPlaying()) {
                if (vlcMutex.try_lock()) {
                    if (needUpdate) {
                        // Supposed to construct the video output as a openGL texture.
                        // Doesn't seem to be working. By dumping the pixel buffer to a
                        // file, and a python script to re-assemble the bytes into RGB
                        // pixels and save the resulting image, I can confirm that
                        // pixelBuffer does correctly hold the video data, meaning that the
                        // problem is entirely in rendering with opengl/imgui.
                        ...
                        LoadTextureFromData((const char*)pixelBuffer, &videoTextureId,
                            &video_image_width, &video_image_height);
                        needUpdate = false;
                    }
                    vlcMutex.unlock();
                }
            ...
            // // Well, the code copied from the ImGui tutorial works as expected. I
            // guess that tells us... something.
            // ImGui::Image((void*)(intptr_t)test_image_texture,
            // ImVec2(test_image_width, test_image_height));
