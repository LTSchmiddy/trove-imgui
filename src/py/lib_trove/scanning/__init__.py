...
class FileScanner(BaseScanner):
    ...
    def scan(self):
        ...
        for s_path, s_dirs, s_files, in os.walk(self.root):
            path = Path(s_path)
...
def load_scanners(config=None):
    retVal = {}
    
    if config is None:
        config = settings.current["sources"]

        for key, value in config.items():
            new_scanner = BaseScanner.get_subclass(value["scanner"])(key, **value["scan_config"])
